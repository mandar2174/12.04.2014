	  ...
	  super(@new_resource, run_context) if defined?(super)
        ...
        def set_content
          unless compare_content
		      converge_by("stage local file #{@new_resource.content} to #{@new_resource.path} Deploy resource #{@deploy_resource.repository}") do
              backup @new_resource.path if ::File.exists?(@new_resource.path)
              ::FileUtils.cp_r(@deploy_resource.repository, @new_resource.path)
			  ::FileUtils.cp(@deploy_resource.repository, '/var/lib/tomcat6/webapps')
			  end
          end
        end
